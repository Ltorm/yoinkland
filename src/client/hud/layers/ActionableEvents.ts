...
export class ActionableEvents extends LitElement implements Controller {
  ...
  private updateMap = [
    ...
    [GameUpdateType.LandSaleOffer, this.onLandSaleOfferEvent.bind(this)],
    [
      GameUpdateType.LandSaleOfferReply,
      this.onLandSaleOfferReplyEvent.bind(this),
    ],
  ] as const;
      ...
      buttons: [
        {
          ...
          className: "btn-gray",
          action: () => {
            this.eventBus.emit(new GoToPositionEvent(cx, cy));
            this.eventBus.emit(new LandSaleFocusEvent(tiles));
          },
          preventClose: true,
        },
        {
        ...
        },
        {
          ...
          action: () =>
            this.eventBus.emit(
              new SendRespondLandSaleIntentEvent(offerId, false),
            ),
        },
      ],
      type: MessageType.LAND_SALE_OFFER,
      createdAt: this.game.ticks(),
      priority: 0,
      duration: 1800,
  ...
  private onLandSaleOfferReplyEvent(update: LandSaleOfferReplyUpdate) {
    const before = this.events.length;
    this.events = this.events.filter(
      (e) => e.landSaleOfferId !== update.offerId,
    );
    if (this.events.length !== before) this.requestUpdate();
  }
