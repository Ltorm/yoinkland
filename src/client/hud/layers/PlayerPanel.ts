...
export class PlayerPanel extends LitElement implements Controller {
  ...
  // Shows "Your lord" / "Your vassal" when a vassal relationship exists between
  // the viewing player and the panel's target (driven by server interaction
  // flags, so no client relationship sync is needed).
  private renderVassalBadge() {
    const interaction = this.actions?.interaction;
    const key = interaction?.targetIsMyLord
      ? "player_panel.your_lord"
      : interaction?.canReleaseVassal
        ? "player_panel.your_vassal"
        : null;
    if (key === null) return html``;
    return html`
      <div class="mt-1" role="status">
        <span
          class="inline-flex items-center gap-2 rounded-full border border-amber-300/30
            bg-amber-400/10 px-2.5 py-0.5 text-sm font-semibold text-amber-100"
          title=${translateText(key)}
        >
          <img
            src=${surrenderIcon}
            alt=""
            ...
            class="size-4.5"
          />
          <span class="tracking-tight">${translateText(key)}</span>
        </span>
      </div>
    `;
  }
  ...
  private renderIdentityRow(other: PlayerView, my: PlayerView) {
    ...
    return html`
      ...
      ${this.renderTraitorBadge(other)} ${this.renderVassalBadge()}
