...
export class GPURenderer {
  ...
  constructor(
    ...
    const gl = canvas.getContext("webgl2", {
      // Transparent so the in-game background image shows in the area beyond
      // the map (the void). The map itself is drawn fully opaque.
      alpha: true,
      antialias: false,
  ...
  private renderFrame(): void {
    ...
    // Restore full alpha writes for the next frame's clear / other GL work.
  ...
  private drawBaseLayer(cam: Float32Array): void {
    ...
    // Clear the void to fully transparent (alpha 0) so the DOM background shows
    // through there. The terrain writes alpha 1, so the map stays opaque.
    gl.colorMask(true, true, true, true);
    gl.clearColor(60 / 255, 60 / 255, 60 / 255, 0.0);
    gl.clear(gl.COLOR_BUFFER_BIT);
    gl.disable(gl.BLEND);
    if (pe.terrain) this.terrainPass.draw(cam);
    // Stop writing alpha from here on, so blended overlays (territory, etc.)
    // can't erode the map's opacity and let the background bleed through.
