...
export class StructurePass {
  ...
  constructor(
    ...
    // The Trebuchet has no dedicated atlas sprite yet — render it with the
    // Missile Silo's icon (both are launch structures).
    const siloCol = this.typeToAtlasCol.get(UT_MISSILE_SILO);
    if (siloCol !== undefined) {
      this.typeToAtlasCol.set("Trebuchet", siloCol);
    }
