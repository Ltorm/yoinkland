      ...
      >
        ...
        <button
          class="nav-menu-item ${currentPage === "page-play"
            ? "active"
            : ""} text-amber-100/80 hover:text-yellow-300 font-medium tracking-wider uppercase cursor-pointer transition-colors [&.active]:text-yellow-300"
          data-page="page-play"
          data-i18n="main.play"
        ></button>
        ...
        <button
          class="nav-menu-item text-amber-100/80 hover:text-yellow-300 font-medium tracking-wider uppercase cursor-pointer transition-colors [&.active]:text-yellow-300"
        ...
        ></button>
        <button
          class="no-crazygames nav-menu-item hidden text-white/70 hover:text-blue-500 font-medium tracking-wider uppercase cursor-pointer transition-colors [&.active]:text-blue-500"
          data-page="page-clan"
          data-i18n="main.clans"
        ></button>
        <!-- Sponsors dropdown (hover) -->
        <div class="relative group">
          <button
            class="text-amber-100/80 hover:text-yellow-300 font-medium tracking-wider uppercase cursor-pointer transition-colors flex items-center gap-1"
          >
            <span>Sponsors</span>
            <svg
              class="w-3 h-3"
          ...
          <div
            class="absolute left-1/2 -translate-x-1/2 top-full pt-2 hidden group-hover:block z-50"
          >
            <div
              class="flex flex-col bg-zinc-900/95 backdrop-blur-md border border-white/10 rounded-lg py-1 min-w-[190px] shadow-xl"
            >
              ${SPONSORS.map(
                (s) =>
                  html`<a
                    ...
                    class="px-4 py-2 text-sm text-amber-100/80 hover:text-yellow-300 hover:bg-white/5 whitespace-nowrap transition-colors"
                    >${s.name}</a
                  >`,
              )}
            </div>
