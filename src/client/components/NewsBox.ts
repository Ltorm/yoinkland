...
export class NewsBox extends LitElement {
  // When true, render as a horizontally-scrolling ticker (marquee) of all news
  // items instead of the cycling box. Used for the home-page bottom ticker.
  @property({ type: Boolean }) ticker = false;
  ...
  private startCycle() {
    this.stopCycle();
    // The ticker scrolls via CSS; no index cycling needed.
    if (!this.ticker && this.items.length > 1) {
  ...
  render() {
    ...
    if (this.ticker) return this.renderTicker();
  ...
  // Bottom-of-page marquee: all news items scrolling right-to-left. The track is
  // duplicated so the CSS translateX(-50%) loop is seamless; hover pauses it.
  private renderTicker() {
    const renderItem = (item: NewsItem) => {
      ...
      const color =
        typeLabelColors[item.type] ?? typeLabelColors["announcement"];
      const title = item.url
        ? html`<a
            href="${item.url}"
            target="_blank"
            rel="noopener noreferrer"
            class="text-sm font-medium text-white hover:text-blue-300 transition-colors"
            >${item.title}</a
          >`
        : html`<span class="text-sm font-medium text-white"
            >${item.title}</span
          >`;
      return html`<span class="inline-flex items-center gap-2 mx-6">
        <span
          class="shrink-0 text-[10px] font-bold tracking-wider px-2 py-0.5 rounded ${color}"
          >${label}</span
        >
        ${title}
    ...
    return html`
      <div
        class="yl-ticker-bar w-full overflow-hidden bg-surface/90 border-t border-white/10 py-2"
      >
        <div class="yl-ticker-track">
          ${this.items.map(renderItem)}${this.items.map(renderItem)}
        </div>
      </div>
    `;
  }
