...
export class SendReleaseVassalIntentEvent implements GameEvent {
  constructor(
    public readonly lord: PlayerView,
    public readonly vassal: PlayerView,
  ) {}
...
export class BuildLandBridgeIntentEvent implements GameEvent {
  constructor(
    public readonly startTile: TileRef,
    public readonly targetTile: TileRef,
  ) {}
}

export class FireTrebuchetIntentEvent implements GameEvent {
  constructor(
    public readonly trebuchetId: number,
    public readonly targetTile: TileRef,
  ) {}
}
...
export class SendRespondLandSaleIntentEvent implements GameEvent {
  constructor(
...
export class Transport {
  ...
  constructor(
    ...
    this.eventBus.on(SendSurrenderIntentEvent, (e) => this.onSendSurrender(e));
    this.eventBus.on(SendReleaseVassalIntentEvent, (e) =>
      this.onSendReleaseVassal(e),
    );
    ...
    this.eventBus.on(SendProposeLandSaleIntentEvent, (e) =>
      this.onProposeLandSale(e),
    );
    ...
    this.eventBus.on(BuildLandBridgeIntentEvent, (e) =>
      this.onBuildLandBridgeIntent(e),
    );
  ...
  private onSendSurrender(event: SendSurrenderIntentEvent) {
    this.sendIntent({
      type: "surrender",
      lord: event.lord.id(),
    });
  ...
  private onSendReleaseVassal(event: SendReleaseVassalIntentEvent) {
    this.sendIntent({
      type: "release_vassal",
      vassal: event.vassal.id(),
    });
  }
  ...
  private onBuildLandBridgeIntent(event: BuildLandBridgeIntentEvent) {
    this.sendIntent({
      type: "build_land_bridge",
      startTile: event.startTile,
      targetTile: event.targetTile,
    });
  }

  private onFireTrebuchetIntent(event: FireTrebuchetIntentEvent) {
    this.sendIntent({
      type: "fire_trebuchet",
      trebuchetId: event.trebuchetId,
      targetTile: event.targetTile,
    });
  }
  ...
  private onRespondLandSale(event: SendRespondLandSaleIntentEvent) {
    this.sendIntent({
