...
// Fired when "Land Bridge" build mode is toggled (B key / radial / Escape /
// right-click-cancel / placed).
export class LandBridgeModeEvent implements GameEvent {
  constructor(public readonly enabled: boolean) {}
}

// Fired when "Sell Land" lasso mode is toggled (radial / Escape / sent).
export class SellLandModeEvent implements GameEvent {
  constructor(public readonly enabled: boolean) {}
}
...
export class InputHandler {
  ...
  initialize() {
    ...
    window.addEventListener("keydown", (e) => {
      ...
      if (e.code === "KeyB" && !e.repeat) {
        e.preventDefault();
        this.uiState.landBridgeMode = !this.uiState.landBridgeMode;
        if (this.uiState.landBridgeMode) {
          // Land-bridge mode and structure ghosts are mutually exclusive.
          this.setGhostStructure(null);
        }
        this.eventBus.emit(
          new LandBridgeModeEvent(this.uiState.landBridgeMode),
        );
      }
  ...
  onPointerUp(event: PointerEvent) {
    ...
    if (dist < this.DRAG_THRESHOLD_PX) {
      ...
      if (
        ...
        this.uiState.landBridgeMode // Left-click places the land bridge
      ) {
  ...
  private onPointerMove(event: PointerEvent) {
    ...
    if (this.pointers.size === 1) {
      ...
      } else if (this.uiState.trebuchetAiming || this.uiState.sellLandMode) {
        // Aiming a trebuchet / drawing a sell-land lasso — track the drag, no pan.
  ...
  private onContextMenu(event: MouseEvent) {
    ...
    if (this.uiState.landBridgeMode) {
      // Right-click cancels land-bridge placement (like canceling a structure
      // ghost). Left-click places it.
      this.uiState.landBridgeMode = false;
      this.eventBus.emit(new LandBridgeModeEvent(false));
      return;
    }
