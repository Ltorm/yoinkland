...
export interface UnitParamsMap {
  ...
  [UnitType.Trebuchet]: Record<string, never>;
...
export class PlayerInfo {
  ...
  constructor(
    ...
    // Matchmaking party id. Members of the same party are kept strictly together
    // on one team (like a clan) — this is how a queued group of friends stays
    // together in a group-vs-group match.
    public readonly party: string | null = null,
  ) {
...
export interface Player {
  ...
  // Vassalage (one-way, unconditional surrender). A vassal can never attack or
  // nuke its lord or the lord's allies; the lord may take the vassal's land
  // freely with no debuff. Ends only when the lord or the vassal is eliminated.
  vassalLord(): Player | null;
  vassals(): Player[];
  isVassalOf(other: Player): boolean;
  canSurrenderTo(other: Player): boolean;
  surrenderTo(lord: Player): void;
  canReleaseVassal(other: Player): boolean;
...
export interface Game extends GameMap {
  ...
  // Land sales — pending offers from a seller to a bordering buyer. Returns the
  // new offer's id.
  createLandSaleOffer(
    seller: Player,
    buyer: Player,
  ...
  /**
   * Convert a water tile to land — the inverse of a nuke crater, used to build
   * land bridges. The tile must currently be water and not impassable. On
   * success it becomes (unowned) plains and the surrounding shoreline, minimap
   * and water-navigation state are updated; the caller is responsible for
   * conquering the new tile. Returns true if the tile was converted.
   */
  convertToLand(tile: TileRef): boolean;
}
...
export interface PlayerInteraction {
  ...
  canEmbargo: boolean;
  // True when the player may surrender to the target (target becomes lord).
  canSurrender: boolean;
  // True when the target is the player's vassal (player may release them).
  canReleaseVassal: boolean;
  // True when the target is the player's lord (the player has surrendered to it).
  targetIsMyLord: boolean;
...
export const MESSAGE_TYPE_CATEGORIES: Record<MessageType, MessageCategory> = {
  ...
  [MessageType.LAND_SALE_OFFER]: MessageCategory.TRADE,
} as const;
