...
export class WaterManager {
  ...
  /**
   * Convert a single water tile to land (the inverse of the nuke water
   * conversion) — the core of land-bridge building. Sets the land terrain,
   * fixes shoreline bits across the tile's 2-ring, updates the minimap and
   * marks the water navigation graph dirty when the new land closes a strait so
   * boats re-path around it. Returns the tiles whose terrain changed (for client
   * tile updates). The tile is left UNOWNED — the caller conquers it.
   *
   * Note: because the navigation graph runs on the half-resolution minimap, a
   * 1-tile-wide bridge may not register as a boat barrier (only its land/sea
   * footprint is tracked). Wider bridges block boats as expected.
   */
  convertToLand(tile: TileRef, magnitude: number): TileRef[] {
    if (!this.map.isWater(tile) || this.map.isImpassable(tile)) return [];

    this.map.setLand(tile, magnitude);
    ...
    // Inline neighbor helper (no allocation, cardinal only) — mirrors
    // finalizeWaterChanges so shoreline logic stays consistent.
    const pushNeighbors = (
      t: TileRef,
      out: TileRef[],
      start: number,
    ): number => {
      if (t >= w) out[start++] = (t - w) as TileRef;
      if (t < totalTiles - w) out[start++] = (t + w) as TileRef;
      const x = t % w;
      if (x > 0) out[start++] = (t - 1) as TileRef;
      if (x < w - 1) out[start++] = (t + 1) as TileRef;
      return start;
    };
    const nb: TileRef[] = new Array(8);

    // ── Shoreline fixup over the converted tile and its 2-ring ──────
    ...
    for (let i = 0; i < end; i++) {
      ...
      for (let j = end; j < end2; j++) tilesToCheck.add(nb[j]);
    }
    for (const t of tilesToCheck) {
      if (map.isImpassable(t)) {
        if (map.isShoreline(t)) {
          map.clearShorelineBit(t);
          changed.add(t);
        }
        continue;
      }
      const tIsLand = map.isLand(t);
      let hasOpposite = false;
      const e = pushNeighbors(t, nb, 0);
      for (let i = 0; i < e; i++) {
        if (map.isImpassable(nb[i])) continue;
        if (map.isLand(nb[i]) !== tIsLand) {
          hasOpposite = true;
          break;
        }
      }
      const oldShoreline = map.isShoreline(t);
      if (hasOpposite) {
        if (!oldShoreline) {
          map.setShorelineBit(t);
          changed.add(t);
        }
      } else if (oldShoreline) {
        map.clearShorelineBit(t);
        changed.add(t);
      }
    }

    // ── Minimap + navigation graph ─────────────────────────────────
    // Flip the owning 2x2 mini tile to land once it's mostly land, then mark
    // the water graph dirty so the throttled rebuild excludes it.
        ...
        for (let dy = 0; dy < 2; dy++) {
          for (let dx = 0; dx < 2; dx++) {
            if (map.isValidCoord(fx + dx, fy + dy)) {
              totalCount++;
              if (map.isLand(map.ref(fx + dx, fy + dy))) landCount++;
            }
          }
        }
        if (landCount >= Math.min(3, totalCount)) {
          this.miniMap.setLand(miniTile, magnitude);
