...
// Terrain magnitude assigned to freshly-built land-bridge tiles. Low magnitude
// renders as plains; kept below IMPASSABLE_MAGNITUDE so the tile is ownable.
const LAND_BRIDGE_MAGNITUDE = 5;
...
export class GameImpl implements Game {
  ...
  private landSaleOffers = new Map<number, LandSaleOffer>();
  private nextLandSaleOfferId = 1;
  ...
  convertToLand(tile: TileRef): boolean {
    if (!this.isWater(tile) || this.isImpassable(tile)) return false;
    if (this.hasOwner(tile)) {
      throw Error(`cannot convert to land, tile ${tile} has owner`);
    }
    const changed = this._waterManager.convertToLand(
      tile,
      LAND_BRIDGE_MAGNITUDE,
    );
    for (const t of changed) {
      this.recordTileUpdate(t);
    }
  ...
  createLandSaleOffer(
    ...
    const offerId = this.nextLandSaleOfferId++;
    const offer: LandSaleOffer = {
      offerId,
      seller,
      buyer,
      recipient,
    ...
    this.landSaleOffers.set(offerId, offer);
    this.addUpdate({
      type: GameUpdateType.LandSaleOffer,
      offerId,
