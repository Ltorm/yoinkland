...
export class GameMapImpl implements GameMap {
  ...
  // Inverse of setWater: convert a water tile into land of the given magnitude.
  // Clears the ocean/shoreline bits (shoreline is recomputed by the caller).
  // Magnitude is clamped below IMPASSABLE so the new tile is always ownable.
  setLand(ref: TileRef, magnitude: number): void {
    if (this.isLand(ref)) return;
    const mag = Math.min(
      magnitude & GameMapImpl.MAGNITUDE_MASK,
      GameMapImpl.IMPASSABLE_MAGNITUDE - 1,
    );
    this.terrain[ref] = (1 << GameMapImpl.IS_LAND_BIT) | mag;
    this.numLandTiles_++;
  }
