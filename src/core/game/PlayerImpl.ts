...
export class PlayerImpl implements Player {
  ...
  // Vassalage: the lord this player has surrendered to (one-way), and the set
  // of players who have surrendered to this one.
  private _vassalLord: PlayerImpl | null = null;
  public _vassals = new Set<PlayerImpl>();
  ...
  // ── Vassalage (one-way surrender) ──────────────────────────────────────
  vassalLord(): Player | null {
    // A dead lord no longer holds the vassal.
    if (this._vassalLord !== null && this._vassalLord.isAlive()) {
      return this._vassalLord;
    }
    return null;
  }

  vassals(): Player[] {
    return [...this._vassals].filter(
      (v) => v.isAlive() && v.vassalLord() === this,
    );
  }

  isVassalOf(other: Player): boolean {
    return this.vassalLord() === other;
  }
  ...
  surrenderTo(lord: Player): void {
    if (!this.canSurrenderTo(lord)) return;
    // End any alliance between us silently (no traitor mark) so the lord can
    // take our land freely.
    const alliance = this.allianceWith(lord);
    if (alliance !== null) {
      this.mg.expireAlliance(alliance);
    }
    if (this._vassalLord !== null) {
      this._vassalLord._vassals.delete(this);
    }
    this._vassalLord = lord as PlayerImpl;
    (lord as PlayerImpl)._vassals.add(this);
  }

  canReleaseVassal(other: Player): boolean {
    return other.isVassalOf(this);
  }

  releaseVassal(vassal: Player): void {
    const v = vassal as PlayerImpl;
    this._vassals.delete(v);
    if (v._vassalLord === this) {
      v._vassalLord = null;
    }
  }
  ...
  public canAttackPlayer(
    ...
    // A vassal can never attack its lord or any of the lord's allies.
    const lord = this.vassalLord();
    if (lord !== null && (player === lord || lord.isAlliedWith(player))) {
      return false;
    }
    if (this.type() !== PlayerType.Human) {
