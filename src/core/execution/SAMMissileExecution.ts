...
export class SAMMissileExecution implements Execution {
  ...
  tick(ticks: number): void {
    ...
    // Mirv warheads are too fast, and mirv shouldn't be stopped ever. Boulders
    // are interceptable too (the SAM only targets ones landing inside its zone).
    const interceptable = [
      UnitType.AtomBomb,
    ...
    if (
      ...
      !interceptable.includes(this.target.type())
    ) {
    ...
    for (let i = 0; i < this.speed; i++) {
      ...
      if (result.status === PathStatus.COMPLETE) {
        ...
        // Record stats (nukes only — boulders aren't a tracked bomb type)
        if (
