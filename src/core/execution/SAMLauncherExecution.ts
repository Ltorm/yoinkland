...
class SAMTargetingSystem {
  ...
  public getSingleTarget(ticks: number): Target | null {
    ...
    // Boulders (lower priority than nukes): only intercept those that will LAND
    // inside the protection zone — never ones merely passing over to land
    // somewhere else. `nearbyUnits` already limits to boulders within range, so
    // the SAM can hit the one it returns at its current position.
    if (best === null) {
      const boulders = this.mg.nearbyUnits(
        samTile,
        range,
        [UnitType.Boulder],
        ({ unit }) => {
          ...
          if (owner === this.sam.owner()) return false;
          if (this.sam.owner().isFriendly(owner)) {
            return (
              this.mg.getWinner() !== null &&
              this.sam.owner().isOnSameTeam(owner)
            );
          }
          const dst = unit.targetTile();
          return (
            dst !== undefined &&
            this.mg.euclideanDistSquared(samTile, dst) <= rangeSquared
          );
        },
      );
      if (boulders.length > 0) {
        let closest = boulders[0];
        for (const b of boulders) {
          if (b.distSquared < closest.distSquared) closest = b;
        }
        best = { unit: closest.unit, tile: closest.unit.tile() };
      }
    }
