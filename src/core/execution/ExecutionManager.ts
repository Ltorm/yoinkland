...
import { FireTrebuchetExecution } from "./FireTrebuchetExecution";
import { LandBridgeExecution } from "./LandBridgeExecution";
import { MarkDisconnectedExecution } from "./MarkDisconnectedExecution";
import { MoveWarshipExecution } from "./MoveWarshipExecution";
import { NationExecution } from "./NationExecution";
import { NoOpExecution } from "./NoOpExecution";
import { PauseExecution } from "./PauseExecution";
import { ProposeLandSaleExecution } from "./ProposeLandSaleExecution";
import { QuickChatExecution } from "./QuickChatExecution";
import { ReleaseVassalExecution } from "./ReleaseVassalExecution";
...
export class Executor {
  ...
  createExec(intent: StampedIntent): Execution {
    ...
    switch (intent.type) {
      ...
      case "surrender":
        return new SurrenderExecution(player, intent.lord);
      case "release_vassal":
        return new ReleaseVassalExecution(player, intent.vassal);
      case "targetPlayer":
      ...
      case "build_land_bridge":
        return new LandBridgeExecution(
          player,
          intent.startTile,
          intent.targetTile,
        );
      case "fire_trebuchet": {
        const treb = this.mg.unit(intent.trebuchetId);
        if (treb === undefined || treb.type() !== UnitType.Trebuchet) {
          return new NoOpExecution();
        }
        return new FireTrebuchetExecution(player, treb, intent.targetTile);
      }
      ...
      case "respond_land_sale":
        return new RespondLandSaleExecution(
          player,
          intent.offerId,
          intent.accept,
        );
