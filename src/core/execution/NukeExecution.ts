...
export class NukeExecution implements Execution {
  ...
  init(mg: Game, ticks: number): void {
    this.mg = mg;
    // A vassal may never nuke its lord or the lord's allies. (Warheads from a
    // MIRV were already validated when the MIRV launched.)
    if (this.nukeType !== UnitType.MIRVWarhead) {
      const target = mg.owner(this.dst);
      const lord = this.player.vassalLord();
      if (
        target.isPlayer() &&
        lord !== null &&
        (target === lord || lord.isAlliedWith(target as Player))
      ) {
        mg.displayMessage(
          "events_display.vassal_cannot_nuke",
