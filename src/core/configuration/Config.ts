...
export class Config {
  ...
  waterNukes(): boolean {
    // On by default for every map; only an explicit `false` disables it.
    return this._gameConfig.waterNukes ?? true;
  }
  ...
  // ── Trebuchet / Boulder tuning ─────────────────────────────────────────
  // Ticks between trebuchet shots (10 ticks = 1s).
  trebuchetCooldown(): number {
    return 10 * 10;
  }
