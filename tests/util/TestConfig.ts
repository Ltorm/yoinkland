...
export class TestConfig extends Config {
  ...
  // Keep the historical test default (off unless a test opts in), so existing
  // nuke tests are unaffected by the production "on by default" change.
  waterNukes(): boolean {
    return this.gameConfig().waterNukes ?? false;
  }
